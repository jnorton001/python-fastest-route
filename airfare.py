...
def find_cheapest_route(current_route=[1], current_cost=0):
    # displays where the program is at
    # uncomment the next line to track script (slows down script significantly)
    # print route

    # if route is complete, return the results
    if len(current_route) == 15:
    ...
    for city, cost in routes[current_route[-1]].iteritems():
        # only evaluate next step if not already in route
        if city not in current_route:
            # recursive call
            new_cost, new_route = find_cheapest_route(current_route + [city],
                                                      current_cost + cost)
...
def main():
    ...
    time_taken = datetime.datetime.now() - start_time  # calculate take taken

    # display results
    print 'Cheapest Airfare Cost: %s' % cost
    print 'Route Taken: %s' % route
    print 'Time spent computing this result: %s' % time_taken
